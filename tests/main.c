...
void *thread_routine(void *data)
{
    ...
    pthread_t tid;
    t_counter *counter; // pointeur vers la variable dans le main
    unsigned int i;

    tid = pthread_self();
    counter = (t_counter *)data;
    // On imprime le compte avant que ce thread commence
    // a itérer, on verrouile la valeur le mutex pour lire la valeur du count
    pthread_mutex_lock(&counter->count_mutex);
    printf("%sThread [%ld]: compte au depart = %u.%s\n", YELLOW, tid, counter->count, NC);
    // le deverouille juste apres la lecture
